...
update_z <- function(Y0, X0, W_tilde,
  ...
  z_new <- z
  w_pseudo <- sapply(1:Lambdai, function(j) alpha[1,j] * W_tilde + (1-alpha[1,j]) * w_t0) ## length of Lambdai list
...
update_w <- function(Y0, X0, W_tilde,,
  ...
  for(t in 1:T){
    ...
    if(t == t0){
      ...
      for(k in 1:K){
        mu_wtk <- rowSums(sapply(1:Lambdai, function(j) tau_x[,j] * rowSums(Xt[, Cl == k]) + gamma[[j]][,k] * v[[j]][,k] * tau_w[1,j]))/tau_wt
